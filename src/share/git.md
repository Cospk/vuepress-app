...
![](https://gavvy-cloud.oss-cn-shenzhen.aliyuncs.com/web/git_work.png)
...
PR（pull  request）：合并请求，如开发完成需要将代码合并到测试分支进行测试，或者合并到main分支

code  review：代码评审，对于合并的代码需要严格的审核代码质量，有同意合并、只给出评论已经给出修改意见进行修改后重新PR一次
...
这是 Git 仓库的根目录，通常是一个隐藏文件夹，位于项目的根目录下。它的存在标志着当前目录是一个 Git 仓库。

**2. `.git/objects/`**

存储 Git 对象（如提交、树、blob 等）的文件。这些对象是 Git 版本控制的核心数据。

- **`pack/`**：存储打包的对象文件（`.pack` 和 `.idx` 文件），用于优化存储和性能。
- **其他文件**：以 SHA-1 哈希值命名的对象文件，分为两个层级：
  - 第一层是对象哈希值的前两位（如 `01`）。
  - 第二层是对象哈希值的剩余部分（如 `01/abcdef1234567890abcdef1234567890abcdef12`）。

**3. `.git/refs/`**

存储引用（refs）的文件，这些引用指向仓库中的特定对象（如提交、分支、标签等）。

- **`heads/`**：存储本地分支的引用。每个分支对应一个文件，文件名是分支名，内容是分支指向的提交的哈希值。
  - 示例：`.git/refs/heads/main` 文件存储 `main` 分支的提交哈希值。
- **`tags/`**：存储标签的引用。每个标签对应一个文件，文件名是标签名，内容是标签指向的对象的哈希值。
  - 示例：`.git/refs/tags/v1.0` 文件存储标签 `v1.0` 的提交哈希值。
- **`remotes/`**：存储远程分支的引用。每个远程仓库对应一个目录，目录下存储远程分支的引用。
  - 示例：`.git/refs/remotes/origin/main` 文件存储远程仓库 `origin` 的 `main` 分支的提交哈希值。

**4. `.git/HEAD`**

一个文本文件，存储当前工作目录所指向的分支或提交的引用。通常内容是 `ref: refs/heads/main`，表示当前工作目录指向 `main` 分支。

**5. `.git/config`**

存储 Git 仓库的配置信息，包括用户信息、远程仓库地址、分支配置等。这是一个 INI 格式的文件。

**6. `.git/description`**

一个文本文件，通常包含仓库的描述信息。默认内容是 `Unnamed repository; edit this file to name it`。

**7. `.git/hooks/`**

存储 Git 钩子脚本的目录。Git 钩子是在特定事件发生时自动执行的脚本，如 `pre-commit`、`post-commit`、`pre-receive` 等。

- 常见钩子文件：
  - `pre-commit`：在提交之前执行。
  - `post-commit`：在提交之后执行。
  - `pre-receive`：在推送代码到远程仓库之前执行。
  - `post-receive`：在推送代码到远程仓库之后执行。
- 默认情况下，这些钩子文件是示例脚本（以 `.sample` 结尾），需要删除 `.sample` 后缀并编辑脚本内容才能启用。

**8. `.git/index`**

存储当前工作目录的状态信息，包括文件的路径、哈希值、修改时间等。它是 Git 用于快速比较工作目录和仓库状态的核心文件。

**9. `.git/info/`**

存储一些仓库的辅助信息。

- **`exclude`**：存储全局忽略文件的规则（类似于 `.gitignore`，但作用于整个仓库）。
- **`attributes`**：存储 Git 属性文件，用于定义文件的特殊行为（如换行符处理）。

**10. `.git/logs/`**

存储 Git 仓库的变更日志，用于记录分支和引用的变更历史。

- **`HEAD`**：记录 `HEAD` 的变更历史。
- **`refs/`**：存储分支和标签的变更日志，每个分支或标签对应一个日志文件。

**11. `.git/ORIG_HEAD`**

在某些操作（如 `git reset`、`git merge` 等）后，存储原始 `HEAD` 的状态，用于回溯或调试。

**12. `.git/COMMIT_EDITMSG`**

存储最后一次提交时的提交信息。每次执行 `git commit` 时，Git 会将提交信息保存到这个文件中。

**13. `.git/MERGE_HEAD`**

在执行合并操作（如 `git merge` 或 `git pull`）时，存储合并目标分支的提交哈希值。
...
> 在合并分支的时候，若是同一个文件的同一行代码进行修改了，那就会产生冲突conflict
